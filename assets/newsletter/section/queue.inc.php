...
/*
	...
	$Id: queue.inc.php 520 2011-03-11 20:58:47Z matt.simpson $
*/
	...
	case "remove" :
		if((isset($_POST["remove"])) && (is_array($_POST["remove"])) && (count($_POST["remove"]))) {
			if((!isset($_POST["confirmed"])) || (!(int) $_POST["confirmed"])) {
				...
				<?php echo display_notice(array("Please confirm that you wish to delete the following <strong>".count($_POST["remove"])." message".((count($_POST["remove"]) != 1) ? "s" : "")."</strong> from ListMessenger.")); ?>
		...
		if($results) {
			foreach($results as $result) {
				...
				if(!$db->Execute($query)) {
					...
					$NOTICE++;
					$NOTICESTR[] = "It appears as though queue id <strong>".(int) $result["queue_id"]."</strong> has stalled, and ListMessenger Light does not have resume sending capabilities.";
				}
			}
		}
		?>
			...
			<tbody>
				...
				<tr>
					<td colspan="5" style="border-top: 1px #333333 dotted; padding-top: 5px">
						...
						<input type="submit" value="Delete Selected" class="button" style="vertical-align: middle" />
					</td>
				</tr>
			</tbody>
			</table>
