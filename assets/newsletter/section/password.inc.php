...
/*
	...
	$Id: login.inc.php 143 2008-02-09 16:41:48Z matt.simpson $
*/
...
if((bool) $_SESSION["isAuthenticated"])	{
	$NOTICE++;
				...
				if(isset($_GET["hash"])) {
									...
									if((isset($_POST["npassword2"])) && (trim($_POST["npassword2"]))) {
										if(trim($_POST["npassword1"]) == trim($_POST["npassword2"])) {
											if(strlen(trim($_POST["npassword1"])) > 5) {
												if($db->AutoExecute(TABLES_PREFIX."preferences", array("preference_value" => md5(trim($_POST["npassword1"]))), "UPDATE", "preference_id='".PREF_ADMPASS_ID."'")) {
													@reload_configuration();

													$db->AutoExecute(TABLES_PREFIX."preferences", array("preference_value" => ""), "UPDATE", "preference_id='".PASSWORD_RESET_HASH."'");
					...
					switch($STEP) {
						case 2 :
							...
							if($db->AutoExecute(TABLES_PREFIX."preferences", array("preference_value" => $hash), "UPDATE", "preference_id='".PASSWORD_RESET_HASH."'")) {
