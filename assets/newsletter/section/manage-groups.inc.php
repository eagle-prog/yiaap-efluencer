...
/*
	...
	$Id: manage-groups.inc.php 481 2009-11-29 16:21:11Z matt.simpson $
*/
...
switch($ACTION) {
	case "add" :
		if($_POST) {
			...
			if(!$ERROR) {
				$query = "INSERT INTO `".TABLES_PREFIX."groups` (`groups_id`, `group_name`, `group_parent`, `group_private`) VALUES (NULL, '".checkslashes($_POST["group_name"])."', '".checkslashes($_POST["group_parent"])."', '".(($_POST["group_private"] == "1") ? "true" : "false")."')";
				if($db->Execute($query)) {
		...
		<tbody>
			<tr>
				...
				<td><input type="text" class="text-box" style="width: 200px" name="group_name" value="<?php echo checkslashes($_POST["group_name"], 1); ?>" /></td>
			</tr>
			<tr>
				...
				<td>
					...
					<?php echo groups_inselect(0, array(checkslashes($_POST["group_parent"]))); ?>
					</select>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					<select id="group_private" name="group_private" style="width: 204px">
						<option value="0"<?php echo ((isset($_POST["group_private"])) ? (($_POST["group_private"] != "1") ? " selected=\"selected\"" : "") : " selected=\"selected\""); ?>>Public Group</option>
						<option value="1"<?php echo ((isset($_POST["group_private"])) ? (($_POST["group_private"] == "1") ? " selected=\"selected\"" : "") : ""); ?>>Private Group</option>
					</select>
				</td>
			</tr>
		</tbody>
				...
				<tbody>
					...
					<tr>
						...
						<td>
							...
							<option value="0">-- Top Level Group --</option>
							<?php echo groups_inselect(0, array((($_POST) ? checkslashes($_POST["group_parent"]) : $result["group_parent"])), 0, array($_GET["id"])); ?>
							</select>
						</td>
					</tr>
					<tr>
