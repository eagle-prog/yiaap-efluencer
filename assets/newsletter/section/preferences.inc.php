...
/*
	...
	$Id: preferences.inc.php 517 2011-03-10 15:51:25Z matt.simpson $
*/
...
switch($_GET["type"]) {
	case "program" :
		if((isset($_POST["npassword1"])) || (isset($_POST["npassword2"])) || ((isset($_POST["preferences"])) && (is_array($_POST["preferences"])) && (count($_POST["preferences"])))) {			
			...
			<tr>
				...
				<td>
					...
					if(@is_dir($_SESSION["config"][PREF_PROPATH_ID]."javascript/wysiwyg/tiny_mce")) {
						echo "<option value=\"tiny_mce\"".((($_SESSION["config"][PREF_USERTE] == "tiny_mce") || ($_SESSION["config"][PREF_USERTE] == "yes")) ? " selected=\"selected\"" : "").">Enabled [TinyMCE Basic]</option>\n";
						echo "<option value=\"tiny_mce_adv\"".((($_SESSION["config"][PREF_USERTE] == "tiny_mce_adv") || ($_SESSION["config"][PREF_USERTE] == "yes")) ? " selected=\"selected\"" : "").">Enabled [TinyMCE Advanced]</option>\n";
						$available_editors++;
					}
					if(@is_dir($_SESSION["config"][PREF_PROPATH_ID]."javascript/wysiwyg/htmlarea")) {
						echo "<option value=\"htmlarea\"".(($_SESSION["config"][PREF_USERTE] == "htmlarea") ? " selected=\"selected\"" : "").">Enabled [HTMLArea]</option>\n";
						$available_editors++;
					}
					?>
					...
					if(!$available_editors) {
						...
						if(!@is_dir($_SESSION["config"][PREF_PROPATH_ID]."javascript/wysiwyg")) {
							echo display_error(array("Your rich text editor directory does not appear to exist <em>".$_SESSION["config"][PREF_PROPATH_ID]."javascript/wysiwyg</em>.<br /><br />If you would like rich text editor support in ListMessenger, please create this directory and ensure that at least one of the supported rich text editors exists inside of it."));
						} else {
							echo display_notice(array("There does not appear to be any supported rich text editors in <em>".$_SESSION["config"][PREF_PROPATH_ID]."javascript/wysiwyg</em>.<br /><br />If you would like rich text editor support in ListMessenger, please ensure that at least one of the supported rich text editors exists inside of this directory."));
						}
						echo "</div>\n";
					}
					?>
			...
			<tr>
				...
				<td>
					...
					foreach($CHARACTER_SETS as $charset => $description) {
						echo "<option value=\"".$charset."\"".((strtolower($_SESSION["config"][PREF_DEFAULT_CHARSET]) == strtolower($charset)) ? " selected=\"selected\"" : "").">".clean_input($charset, array("upper", "encode")).": ".html_encode($description)."</option>\n";
					}
					?>
					</select>
				</td>
	...
	break;
	case "blacklist" :
		if((isset($_POST["preferences"])) && (is_array($_POST["preferences"])) && (count($_POST["preferences"]))) {
			foreach($_POST["preferences"] as $preference_id => $preference_value) {
				$preference_value	= trim($preference_value);
				$skip_query			= false;
	...
	case "enduser" :
		...
		<tbody>
			...
			if(((!function_exists("gd_info")) || (!function_exists("imagettftext"))) && ($_SESSION["config"][ENDUSER_CAPTCHA] == "yes")) {
				...
				echo "	<td>&nbsp;</td>\n";
				echo "	<td style=\"padding-top: 5px\">\n";
				echo 		display_notice(array("You have setup ListMessenger to require CAPTCHA image confirmation; however, GD Image Library or Freetype is no longer available with PHP on your server.<br /><br />Please disable Require CAPTCHA Image Confirmation, otherwise your subscribers will be unable to subscribe using the end-user tools.<br /><br />If you require this functionality to be enabled (recommended) you must ensure that GD Image Library and Freetype support is available with PHP.</span>"));
				echo "	</td>\n";
				echo "</tr>\n";
			}
			?>
		...
		<tbody>
			...
			if(((!ini_get("allow_url_fopen")) || (ini_get("allow_url_fopen") == "Off")) && ($_SESSION["config"][PREF_FOPEN_URL] == "yes")) {
				...
				echo "	<td>&nbsp;</td>\n";
				echo "	<td style=\"padding-top: 5px\">\n";
				echo 		display_notice(array("You have setup ListMessenger to allow URL fopening; however, your servers' PHP configuration does not allow this.<br /><br />Please disable Allow URL Fopening, otherwise your subscribers will be unable to subscribe using the end-user tools.<br /><br />If you require this functionality to be enabled (i.e. you have PHP template files) then you must enable the <strong>allow_url_fopen</strong> directive in your servers <strong>php.ini</strong> file.</span>"));
				echo "	</td>\n";
				echo "</tr>\n";
			}
			...
			if(@is_dir($language_directory)) {
				...
				if($handle) {
					...
					<tr>
						...
						echo "</select>\n";
						?>
						<span class="small-grey">(Latest language files: <a href="http://www.listmessenger.com/index.php/languages" target="_blank" style="font-size: 11px; font-weight: normal">click here</a>)</span>
						</td>
					</tr>
					<?php
				} else {
		...
		<tbody>
			...
			<tr>
				<td><?php echo create_tooltip("Enable Forward to a Friend", "<strong><em>Enable Forward to a Friend</em></strong><br />If you would like to allow your subscribers to use the <strong>Forward Message to a Friend</strong> feature built into ListMessenger, you can set this to yes.<br /><br />When this feature is enabled subscribers can click a link in the e-mail they receive from you <em>(i.e. [forwardtofriend] variable)</em>, enter their friends contact information into a form on your website, and the message will be sent to their friend.<br /><br /><strong>Important:</strong>This feature is only available in ListMessenger Pro.", false); ?></td>
				<td>
					<select style="width: 175px" name="preferences[<?php echo ENDUSER_FORWARD; ?>]" onkeypress="return handleEnter(this, event)" disabled="true">
					<option value="no" selected="selected">No</option>
					</select>
					<span class="small-grey">(Only available in ListMessenger Pro.)</span>
				</td>
			</tr>
		</tbody>
		</table>
		...
		if((isset($_POST["preferences"])) && (is_array($_POST["preferences"])) && (count($_POST["preferences"]))) {
			foreach($_POST["preferences"] as $preference_id => $preference_value) {
				...
				switch($preference_id) {
					...
					break;
					case PREF_MAILER_LE :
						if(!in_array($preference_value, array("n", "rn", "r"))) {
							$preference_value = "n";
						}
					break;
