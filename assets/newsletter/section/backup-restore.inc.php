...
/*
	...
	$Id: backup-restore.inc.php 524 2011-03-12 04:09:05Z matt.simpson $
*/
...
if(@function_exists("gzopen")) {
	...
	switch($ACTION) {
			...
			if(@is_dir($_SESSION["config"][PREF_PRIVATE_PATH]."tmp/")) {
				if(@is_writable($_SESSION["config"][PREF_PRIVATE_PATH]."tmp/")) {
					...
					foreach($queries as $table => $query) {
						...
						if($results) {
							file_put_contents($bu_filename, "<".$table." records=\"".count($results)."\">".@base64_encode(@serialize($results))."</".$table.">\r\n", FILE_APPEND);
						} else {
		...
		break;
		case "delete" :
			if((is_string($_POST["filename"])) && (trim($_POST["filename"]) != "")) {
			...
			if((is_array($_POST["filename"])) && (count($_POST["filename"]))) {
				if((!isset($_POST["confirmed"])) || ($_POST["confirmed"] != "1")) {
					...
					<?php
					echo display_notice(array("Please confirm that you wish to permenantly remove the following <strong>".count($_POST["filename"])." backup file".((count($_POST["filename"]) != 1) ? "s" : "")."</strong> from ListMessenger."));
					?>
				...
				case "2" :
					if((isset($_POST["filename"])) && (trim($_POST["filename"]) != "")) {
						...
						if(@file_exists($_SESSION["config"][PREF_PRIVATE_PATH]."/backups/".$filename)) {
							...
							if((is_array($_POST["restore"]["tables"])) && (count($_POST["restore"]["tables"]))) {
								...
								if(!$ERROR) {
									...
									if((isset($_POST["skip_path"])) && ($_POST["skip_path"] == "1")) {
							...
							if(@file_exists($_SESSION["config"][PREF_PRIVATE_PATH]."/backups/".$filename)) {
								if((is_array($_POST["restore"]["directories"])) && (count($_POST["restore"]["directories"]))) {
