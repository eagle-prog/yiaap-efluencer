...
/*
	...
	$Id: subscribers.inc.php 529 2011-03-21 02:39:04Z matt.simpson $
*/
...
switch($ACTION) {
	case "add" :
		if($_POST) {
			if(strlen(trim($_POST["email_address"])) < 1) {
				 ...
				 */
				if((isset($_POST["firstname"])) && ($tmp_input = clean_input($_POST["firstname"], array("emailheaders", "notags", "encode", "trim")))) {
				 ...
				 */
				if((isset($_POST["lastname"])) && ($tmp_input = clean_input($_POST["lastname"], array("emailheaders", "notags", "encode", "trim")))) {
				...
				if($results) {
					foreach($results as $result) {
						if(!isset($_POST["cdata"][$result["field_sname"]]) || (!$_POST["cdata"][$result["field_sname"]]) || !custom_data_field_value($_POST["cdata"][$result["field_sname"]])) {
							$ERROR++;
		...
		if(groups_count(0, $groups) < 1) {
			...
			<tbody>
				<tr>
					...
					<td><input type="text" id="email_address" name="email_address" class="text-box" style="width: 200px" value="<?php echo html_encode(checkslashes($_POST["email_address"], 1)); ?>" onkeypress="return handleEnter(this, event)" /></td>
				</tr>
				<tr>
					...
					<td><input type="text" id="firstname" name="firstname" class="text-box" style="width: 200px" value="<?php echo html_encode(checkslashes($_POST["firstname"], 1)); ?>" onkeypress="return handleEnter(this, event)" /></td>
				</tr>
				<tr>
					...
					<td><input type="text" id="lastname" name="lastname" class="text-box" style="width: 200px" value="<?php echo html_encode(checkslashes($_POST["lastname"], 1)); ?>" onkeypress="return handleEnter(this, event)" /></td>
				</tr>
				<tr>
					...
					<td>
						...
						<?php echo groups_inselect(0, ((isset($_POST["group_ids"])) ? $_POST["group_ids"] : "")); ?>
						</select>
					</td>
				</tr>
				<tr>
						...
						switch($result["field_type"]) {
							case "checkbox" :
								if($result["field_options"]) {
									...
									foreach($options as $key => $option) {
										...
										echo "<input type=\"checkbox\" name=\"cdata[".$result["field_sname"]."][]\" id=\"cdata_".$result["field_sname"]."_".$key."\" value=\"".html_encode($pieces[0])."\"".((@in_array($pieces[0], $_POST["cdata"][$result["field_sname"]])) ? " checked=\"checked\"" : "")." onkeypress=\"return handleEnter(this, event)\" /> <label for=\"cdata_".$result["field_sname"]."_".$key."\">".html_encode($pieces[1])."</label><br />\n";
									}
								} else {
							...
							case "radio" :
								if($result["field_options"]) {
									...
									foreach($options as $key => $option) {
										$pieces = explode("=", $option);
										echo "<input type=\"radio\" name=\"cdata[".$result["field_sname"]."]\" id=\"cdata_".$result["field_sname"]."_".$key."\" value=\"".html_encode($pieces[0])."\"".(($_POST["cdata"][$result["field_sname"]] == $pieces[0]) ? " checked=\"checked\"" : "")." onkeypress=\"return handleEnter(this, event)\" /> <label for=\"cdata_".$result["field_sname"]."_".$key."\">".html_encode($pieces[1])."</label><br />\n";
									}
								} else {
							...
							case "select" :
								if($result["field_options"]) {
									...
									foreach($options as $option) {
										$pieces = explode("=", $option);
										echo "<option value=\"".html_encode(trim($pieces[0]))."\"".(($_POST["cdata"][$result["field_sname"]] == $pieces[0]) ? " selected=\"selected\"" : "").">".html_encode(trim($pieces[1]))."</option>\n";
									}
									echo "</select>\n";
								} else {
									echo "<span class=\"small-grey\">(No options in configuration to select)</span>";
							...
							break;
							case "textarea" :
								echo "<textarea name=\"cdata[".$result["field_sname"]."]\" id=\"cdata_".$result["field_sname"]."\" style=\"width: 98%; height: 75px\">".((trim($_POST["cdata"][$result["field_sname"]])) ? html_encode($_POST["cdata"][$result["field_sname"]]) : "")."</textarea>\n";
							break;
							case "textbox" :
								echo "<input type=\"text\" name=\"cdata[".$result["field_sname"]."]\" id=\"cdata_".$result["field_sname"]."\" value=\"".((trim($_POST["cdata"][$result["field_sname"]])) ? html_encode($_POST["cdata"][$result["field_sname"]]) : "")."\"".((strlen($result["field_length"]) > 0) ? " maxlength=\"".$result["field_length"]."\"" : "")." onkeypress=\"return handleEnter(this, event)\" />\n";
							break;
							default :
								echo "&nbsp;";
							break;
				...
				<tr>
					<td>&nbsp;</td>
					<td style="padding-top: 10px"><input type="checkbox" id="confirmation" name="confirmation" value="1"<?php echo (((isset($_POST["confirmation"])) && ($_POST["confirmation"] == "1")) ? " checked=\"checked\"" : ""); ?> style="vertical-align: middle" /> <label for="confirmation" class="form-row-nreq">Send an opt-in confirmation e-mail prior to adding subscriber.</label></td>
				</tr>
			</tbody>
			</table>
			</form>
		...
		switch($STEP) {
			case "2" :
				...
				if(isset($_POST["email_addresses"])) {
					...
					if((isset($_POST["group_ids"])) && (is_array($_POST["group_ids"])) && (@count($_POST["group_ids"]))) {
						foreach($_POST["group_ids"] as $group_id) {
			...
			default :
				...
				<tbody>
					<tr>
						...
						<td><textarea id="email_addresses" name="email_addresses" cols="55" rows="12" style="width: 98%; height: 120px"><?php echo html_encode(checkslashes($_POST["email_addresses"], 1)); ?></textarea></td>
					</tr>
					<tr>
	...
	case "copy" :
		if((isset($_POST["subscribers"])) && (is_array($_POST["subscribers"])) && (count($_POST["subscribers"]))) {
			$subscribers = array();
			...
			if(count($subscribers)) {
				...
				switch($STEP) {
					case "2" :
						if((!isset($_POST["group_ids"])) || (!(int) trim($_POST["group_ids"]))) {
							$ERROR++;
									...
									if($result	= $db->GetRow($query)) {
										...
										if(!$sresult = $db->GetRow($query)) {
											$query	= "
													...
													VALUES ('".(int) trim($_POST["group_ids"])."', '".(((isset($_POST["reset_signup"])) && ($_POST["reset_signup"] == "1")) ? time() : (int) $result["signup_date"])."', ".$db->qstr($result["firstname"]).", ".$db->qstr($result["lastname"]).", ".$db->qstr($result["email_address"]).")";
						...
						<tbody>
							<tr>
								...
								<td>
									...
									<?php echo groups_inselect(0, ((isset($_POST["group_ids"])) ? (int) $_POST["group_ids"] : "")); ?>
									</select>
								</td>
							</tr>
							<tr>
	...
	case "delete" :
		if((isset($_POST["subscribers"])) && (is_array($_POST["subscribers"])) && (count($_POST["subscribers"]))) {
			if((!isset($_POST["confirmed"])) || (!(int) $_POST["confirmed"])) {
				?>
				<h1>Deleting Subscriber<?php echo ((count($_POST["subscribers"]) != 1) ? "s" : "") ?></h1>
				
				<?php
				echo display_notice(array("Please confirm that you wish to delete the following <strong>".count($_POST["subscribers"])." subscriber".((count($_POST["ids"]) != 1) ? "s" : "")."</strong> from ListMessenger."));
				?>
				...
				<tbody>
					<?php
					foreach($_POST["subscribers"] as $users_id) {
						if($users_id = (int) $users_id) {
							$query	= "SELECT `users_id`, `group_id`, `signup_date`, CONCAT_WS(', ', `lastname`, `firstname`) AS `name_field`, `email_address`, `groups_id`, `group_name` FROM `".TABLES_PREFIX."users` LEFT JOIN `".TABLES_PREFIX."groups` ON `".TABLES_PREFIX."users`.`group_id`=`".TABLES_PREFIX."groups`.`groups_id` WHERE `users_id` = ".$db->qstr($users_id, get_magic_quotes_gpc());
							$result	= $db->GetRow($query);
							if($result) {
								echo "<tr>\n";
				...
				if($_POST["updateall"] == "1") {
					...
					if($results) {
						foreach($results as $result) {
							...
							if(trim($_POST["email_address"]) != trim($_POST["oemail_address"])) {
								...
								if($sresult) {
									$NOTICE++;
									$NOTICESTR[] = "The updated e-mail address you have entered already exists in the &quot;".groups_information(array($sresult["group_id"]), true)."&quot; subscriber group. <strong>Skipping</strong>";
								}
							}
							...
							if(!$ERROR) {
								...
								if($db->Execute($squery)) {
									 ...
									 */
									custom_data_store($users_id, (isset($_POST["cdata"]) ? $_POST["cdata"] : array()), $_SESSION["config"]);

									$SUCCESS++;
									$SUCCESSSTR[] = "Successfully updated the account information in group ".groups_information(array($group_id), true).".";
								} else {
									$group_name = groups_information(array($group_id), true);
					...
					if(!$ERROR) {
						...
						if($db->Execute($query)) {
							 ...
							 * Stores users custom field data.
							 */
							custom_data_store((int) trim($_GET["id"]), (isset($_POST["cdata"]) ? $_POST["cdata"] : array()), $_SESSION["config"]);
			...
			<tbody>
				...
				<tr>
					...
					<td>
						...
						<?php echo groups_inselect(0, array((($_POST) ? $_POST["group_id"] : $result["group_id"]))); ?>
						</select>
					</td>
				</tr>
				<tr>
					...
					foreach($results as $result) {
						...
						switch($result["field_type"]) {
							case "checkbox" :
								if($result["field_options"]) {
									$options	= explode("\n", $result["field_options"]);
									$values		= (($cdata) ? explode(", ", $cdata["value"]) : array());

									foreach($options as $option) {
										$pieces = explode("=", $option);
										echo "<input type=\"checkbox\" name=\"cdata[".$result["field_sname"]."][]\" value=\"".html_encode($pieces[0])."\"".((@in_array($pieces[0], (($_POST) ? $_POST["cdata"][$result["field_sname"]] : $values))) ? " checked=\"checked\"" : "")."> ".html_encode($pieces[1])."<br />\n";
									}
								} else {
									echo "<span class=\"small-grey\">(No options in configuration to check)</span>";
								}
							...
							case "radio" :
								if($result["field_options"]) {
									...
									foreach($options as $option) {
										$pieces = explode("=", $option);
										echo "<input type=\"radio\" name=\"cdata[".$result["field_sname"]."]\" value=\"".html_encode($pieces[0])."\"".(((($_POST) ? $_POST["cdata"][$result["field_sname"]] : $cdata["value"]) == $pieces[0]) ? " checked=\"checked\"" : "")."> ".html_encode($pieces[1])."<br />\n";
									}
								} else {
									echo "<span class=\"small-grey\">(No options in configuration to select)</span>";
								}
							...
							case "select" :
								if($result["field_options"]) {
									...
									foreach($options as $option) {
										$pieces = explode("=", $option);
										echo "<option value=\"".html_encode(trim($pieces[0]))."\"".(((($_POST) ? $_POST["cdata"][$result["field_sname"]] : $cdata["value"]) == $pieces[0]) ? " selected=\"selected\"" : "").">".html_encode(trim($pieces[1]))."</option>\n";
									}
									echo "</select>\n";
								} else {
									echo "<span class=\"small-grey\">(No options in configuration to select)</span>";
							...
							break;
							case "textarea" :
								echo "<textarea name=\"cdata[".$result["field_sname"]."]\" style=\"width: 98%; height: 75px\">".html_encode((($_POST) ? ((trim($_POST["cdata"][$result["field_sname"]])) ? $_POST["cdata"][$result["field_sname"]] : "") : $cdata["value"]))."</textarea>\n";
							break;
							case "textbox" :
								echo "<input type=\"text\" name=\"cdata[".$result["field_sname"]."]\" value=\"".html_encode((($_POST) ? ((trim($_POST["cdata"][$result["field_sname"]])) ? $_POST["cdata"][$result["field_sname"]] : "") : $cdata["value"]))."\"".((strlen($result["field_length"]) > 0) ? " maxlength=\"".$result["field_length"]."\"" : "")." />\n";
							break;
							default :
								echo "&nbsp;";
							break;
					...
					case "2" :
						if((!isset($_POST["group_ids"])) || (!(int) trim($_POST["group_ids"]))) {
							...
							if($result) {
								...
								if(@count($subscribers)) {
									$query = "UPDATE `".TABLES_PREFIX."users` SET `group_id` = ".$db->qstr((int) trim($_POST["group_ids"])).(((isset($_POST["reset_signup"])) && ($_POST["reset_signup"] == "1")) ? ", `signup_date` = ".$db->qstr(time()) : "")." WHERE `users_id` IN ('".implode("', '", $subscribers)."')";
									if($db->Execute($query)) {
	...
	break;	// End Move
	case "view" :
		$COLLAPSED	= explode(",", $_COOKIE["display"]["subscribers"]["collapsed"]);
		...
		if($result) {
			...
			<div style="text-align: right">
				...
				<input type="button" value="Close" class="button" onclick="window.location='index.php<?php echo ((($_SESSION["display"]["subscribers"]["lastpage"]) && ((int) $_SESSION["display"]["subscribers"]["lastpage"])) ? "?".replace_query(array("action" => false, "id" => false, "vp" => $_SESSION["display"]["subscribers"]["lastpage"])) : "") ?>'" />
				</form>
			</div>
			<br />
			...
			<?php
				$squery		= "
							...
							LEFT JOIN `".TABLES_PREFIX."groups` AS b
							ON b.`groups_id` = a.`group_id`
							WHERE `email_address` = '".checkslashes($result["email_address"])."'
							AND `group_id` <> ".$db->qstr((int) $result["group_id"]);
			...
			if($results) {
				foreach($results as $result) {
					$squery	= "SELECT * FROM `".TABLES_PREFIX."cdata` WHERE `user_id`='".(int) trim($_GET["id"])."' AND `cfield_id`='".$result["cfields_id"]."'";
					$cdata	= $db->GetRow($squery);
					...
					switch($result["field_type"]) {
						...
						break;
						case "textarea" :
							if($text = trim(nl2br($cdata["value"]))) {
	...
	default :
		...
		} elseif((!isset($_SESSION["display"]["subscribers"]["sort"])) && (isset($_COOKIE["display"]["subscribers"]["sort"]))) {
			$_SESSION["display"]["subscribers"]["sort"] = $_COOKIE["display"]["subscribers"]["sort"];
		} else {
			if(!isset($_SESSION["display"]["subscribers"]["sort"])) {
		...
		} elseif((!isset($_SESSION["display"]["subscribers"]["order"])) && (isset($_COOKIE["display"]["subscribers"]["order"]))) {
			$_SESSION["display"]["subscribers"]["order"] = $_COOKIE["display"]["subscribers"]["order"];
		} else {
			if (!isset($_SESSION["display"]["subscribers"]["order"])) {
				$_SESSION["display"]["subscribers"]["order"] = "ASC";
		...
		if (isset($_GET["pp"]) && ((int) $_GET["pp"] > 0) && ((int) $_GET["pp"] <= 1000)) {
			$perpage = (int) $_GET["pp"];
		} elseif (isset($_SESSION["config"][PREF_PERPAGE_ID]) && ((int) $_SESSION["config"][PREF_PERPAGE_ID] > 0) && ((int) $_SESSION["config"][PREF_PERPAGE_ID] <= 1000)) {
		...
		// Begin Query String
		if((isset($_GET["q"])) && (trim($_GET["q"])) && (isset($_GET["t"])) && (isset($_GET["f"]))) {
			if(isset($_GET["f"])) {
			...
			if(isset($_GET["t"])) {
				switch($_GET["t"]) {
					case "equals" :
						$search_query .= " = ".$db->qstr(trim($_GET["q"]), get_magic_quotes_gpc());
					break;
					case "contains" :
					default :
						$search_query .= " LIKE ".$db->qstr("%".trim($_GET["q"])."%", get_magic_quotes_gpc());
					break;
				}
			}
		}
		
		if((trim($_GET["g"])) && ((int) trim($_GET["g"]))) {
			if($search_query == "") {
				$search_query .= " WHERE";
			} else {
				$search_query .= " AND";
			...
			$groups		= array();
			$groups[]	= (int) trim($_GET["g"]);
			groups_inarray((int) trim($_GET["g"]), $groups);
		...
		if(($totalrows > 175) && ($totalrows <= 200)) {
			echo display_notice(array("Your ListMessenger Light subscriber base is approaching the 200 recommended subscriber limit. This can be stressful on your server because this Light version was not designed to handle lists of more than 200 subscribers. Please upgrade to ListMessenger Pro at: <a href=\"http://www.listmessenger.com\" target=\"_blank\">http://www.listmessenger.com</a>"));
		} elseif($totalrows > 200) {
			echo display_notice(array("You are using ListMessenger Light to manage a subscriber base over 200 subscribers. This can be very stressful on your server because this Light version was not designed to handle lists of this size. Please upgrade to ListMessenger Pro at: <a href=\"http://www.listmessenger.com\" target=\"_blank\">http://www.listmessenger.com</a>"));
		}
		...
		<div style="display: <?php echo (in_array("search", $COLLAPSED) ? "none" : "inline"); ?>" id="opened_search">
			...
			<input type="hidden" name="pp" value="<?php echo ((isset($_GET["pp"])) ? (int) $_GET["pp"] : ""); ?>" />
			<table style="width: 100%; border: 1px #CCCCCC solid" cellspacing="0" cellpadding="1">
			<tr>
					...
					<tr>
						...
						<td>
							<select id="f" name="f" style="vertical-align: middle; width: 150px">
								<?php
								echo "<option value=\"fullname\"".(((isset($_GET["f"])) && ($_GET["f"] == "fullname")) ? " selected=\"selected\"" : "").">Full name</option>\n";
								echo "<option value=\"firstname\"".(((isset($_GET["f"])) && ($_GET["f"] == "firstname")) ? " selected=\"selected\"" : "").">Firstname</option>\n";
								echo "<option value=\"lastname\"".(((isset($_GET["f"])) && ($_GET["f"] == "lastname")) ? " selected=\"selected\"" : "").">Lastname</option>\n";
								echo "<option value=\"email\"".(((isset($_GET["f"])) && ($_GET["f"] == "email")) ? " selected=\"selected\"" : "").">E-Mail Address</option>\n";
								
								$query		= "SELECT `cfields_id`, `field_sname`, `field_lname` FROM `".TABLES_PREFIX."cfields` WHERE `field_type` <> 'linebreak'";
								$results	= $db->GetAll($query);
								if($results) {
									foreach($results as $result) {
										echo "<option value=\"cfield_".$result["cfields_id"]."\"".(((isset($_GET["f"])) && ($_GET["f"] == "cfield_".$result["cfields_id"])) ? " selected=\"selected\"" : "").">".html_encode((($result["field_lname"] != "") ? $result["field_lname"] : $result["field_sname"]))."</option>\n";
									}
								}
								?>
							</select>
							<select name="t" style="vertical-align: middle; width: 85px">
								<option value="contains"<?php echo (((isset($_GET["t"])) && ($_GET["t"] == "contains")) ? " selected=\"selected\"" : ""); ?>>Contains</option>
								<option value="equals"<?php echo (((isset($_GET["t"])) && ($_GET["t"] == "equals")) ? " selected=\"selected\"" : ""); ?>>Equals</option>
							</select>
							<input type="text" class="text-box" style="vertical-align: middle; width: 200px" name="q" value="<?php echo ((isset($_GET["q"])) ? html_encode(checkslashes($_GET["q"], 1)) : ""); ?>" />
						</td>
					...
					<tr>
						...
						<td>
							<select id="g" name="g" style="width: 225px" onchange="$('#search_form').submit();">
								...
								<?php echo groups_inselect(0, array((int) trim($_GET["g"]))); ?>
							</select>
						</td>
						<td style="text-align: right">
							<?php if((isset($_GET["q"])) || (isset($_GET["t"])) || (isset($_GET["f"])) || (isset($_GET["g"]))) : ?>
		...
		<?php
		if((isset($_GET["g"])) && (trim($_GET["g"])) && ($totalrows)) {
			echo "<div style=\"padding: 4px; text-align: right\" class=\"small-grey\">".$totalrows." Subscriber".(($totalrows != 1) ? "s" : "")." in ".groups_information((int) trim($_GET["g"]), true).(((is_array($groups)) && ($total_groups = (count($groups) - 1))) ? " (including ".$total_groups." sub-group".(($total_groups != 1) ? "s" : "").")" : "").".</div>\n";
		}
		...
		if($results) {
			...
			<tr>
				<td style="width: 50%; text-align: left">
					...
					<?php
					echo "<input type=\"hidden\" name=\"pp\" value=\"".((isset($_GET["pp"])) ? (int) $_GET["pp"] : "")."\" />\n";
					echo "<input type=\"hidden\" name=\"q\" value=\"".((isset($_GET["q"])) ? html_encode($_GET["q"]) : "")."\" />\n";
					echo "<input type=\"hidden\" name=\"t\" value=\"".((isset($_GET["t"])) ? html_encode($_GET["t"]) : "")."\" />\n";
					echo "<input type=\"hidden\" name=\"f\" value=\"".((isset($_GET["f"])) ? html_encode($_GET["f"]) : "")."\" />\n";
					echo "<input type=\"hidden\" name=\"g\" value=\"".((isset($_GET["g"])) ? html_encode($_GET["g"]) : "")."\" />";
					?>
			...
			if((strlen($_GET["q"]) > 0) || (strlen($_GET["t"]) > 0) || (strlen($_GET["f"]) > 0) || (strlen($_GET["g"]) > 0)) {
				?>
