...
/*
	...
	$Id: loader.inc.php 481 2009-11-29 16:21:11Z matt.simpson $
*/
	...
	if(isset($_SESSION["config"][PREF_MAILER_BY_ID])) {
		switch(trim($_SESSION["config"][PREF_MAILER_BY_ID])) {
			case "sendmail" :
				...
				if((isset($_SESSION["config"][PREF_MAILER_BY_VALUE])) && (trim($_SESSION["config"][PREF_MAILER_BY_VALUE]) != "")) {
			...
			case "smtp" :
				 ...
				 */
				if(isset($_SESSION["config"][PREF_MAILER_SMTP_KALIVE])) {
					if(in_array(trim($_SESSION["config"][PREF_MAILER_SMTP_KALIVE]), array("yes", "true"))) {
				 ...
				 */
				if(isset($_SESSION["config"][PREF_MAILER_AUTH_ID])) {
					if(in_array(trim($_SESSION["config"][PREF_MAILER_AUTH_ID]), array("yes", "true"))) {
						define("SMTP_AUTH", true);
						
						if((isset($_SESSION["config"][PREF_MAILER_AUTHUSER_ID])) && (trim($_SESSION["config"][PREF_MAILER_AUTHUSER_ID]) != "")) {
							define("SMTP_AUTH_USER", trim($_SESSION["config"][PREF_MAILER_AUTHUSER_ID]));
							
							if((isset($_SESSION["config"][PREF_MAILER_AUTHPASS_ID])) && (trim($_SESSION["config"][PREF_MAILER_AUTHPASS_ID]) != "")) {
