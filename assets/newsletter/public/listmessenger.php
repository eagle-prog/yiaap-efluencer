...
/*
	...
	$Id: listmessenger.php 518 2011-03-10 21:58:15Z matt.simpson $
*/
...
if(!@file_exists($LM_PATH."includes/config.inc.php")) {
	 ...
	 */
	if((isset($LM_REQUEST["addr"])) && (!isset($LM_REQUEST["email_address"]))) {
	...
	if($config[ENDUSER_CAPTCHA] == "yes") {
		if((!isset($LM_REQUEST["captcha_code"])) || (!PhpCaptcha::Validate($LM_REQUEST["captcha_code"]))) {
			$ERROR++;
			...
			if(isset($LM_REQUEST["captcha_code"])) {
				$MESSAGE .= "<thead>\n";
				$MESSAGE .= "	<tr>\n";
				...
				if(!$ERROR) {
					...
					if(is_scalar($LM_REQUEST["group_ids"])) {
						if((int) trim($LM_REQUEST["group_ids"])) {
							$LM_REQUEST["group_ids"] = array((int) trim($LM_REQUEST["group_ids"]));
						} else {
							$LM_REQUEST["group_ids"] = array();
						}
					}
					...
					if(!$ERROR) {
						 ...
						 */ 
						if((isset($LM_REQUEST["email_address"])) && ($LM_REQUEST["email_address"] = clean_input($LM_REQUEST["email_address"], array("nows", "lowercase", "emailheaders")))) {
							if(valid_address($LM_REQUEST["email_address"])) {
								...
								} elseif((is_array($config[ENDUSER_BANIPS])) && (isset($_SERVER["REMOTE_ADDR"])) && (banned_ip($_SERVER["REMOTE_ADDR"], $config[ENDUSER_BANIPS]))) {
									$ERROR++;
						 ...
						 */
						if((isset($LM_REQUEST["firstname"])) && ($tmp_input = public_html_encode(clean_input($LM_REQUEST["firstname"], array("emailheaders", "notags", "trim"))))) {
						 ...
						 */
						if((isset($LM_REQUEST["lastname"])) && ($tmp_input = public_html_encode(clean_input($LM_REQUEST["lastname"], array("emailheaders", "notags", "trim"))))) {
						...
						if($results) {
							foreach($results as $result) {
								if(!isset($LM_REQUEST[$result["field_sname"]]) || (!$LM_REQUEST[$result["field_sname"]]) || !custom_data_field_value($LM_REQUEST[$result["field_sname"]])) {
									$ERROR++;
						...
						if($ERROR) {
							...
							$MESSAGE .= "		<td><input type=\"text\" id=\"email_address\" name=\"email_address\" value=\"".((isset($LM_REQUEST["email_address"])) ? public_html_encode($LM_REQUEST["email_address"]) : "")."\" maxlength=\"128\" /></td>\n";
							$MESSAGE .= "	</tr>\n";
							$MESSAGE .= "	<tr>\n";
							$MESSAGE .= "		<td><label for=\"firstname\"".(($firstname_required) ? " class=\"required\"" : "").">".$LANGUAGE_PACK["page_confirm_firstname"]."</label></td>\n";
							$MESSAGE .= "		<td><input type=\"text\" id=\"firstname\" name=\"firstname\" value=\"".((isset($LM_REQUEST["firstname"])) ? public_html_encode($LM_REQUEST["firstname"]) : "")."\" maxlength=\"32\" /></td>\n";
							$MESSAGE .= "	</tr>\n";
							$MESSAGE .= "	<tr>\n";
							$MESSAGE .= "		<td><label for=\"lastname\"".(($lastname_required) ? " class=\"required\"" : "").">".$LANGUAGE_PACK["page_confirm_lastname"]."</label></td>\n";
							$MESSAGE .= "		<td><input type=\"text\" id=\"lastname\" name=\"lastname\" value=\"".((isset($LM_REQUEST["lastname"])) ? public_html_encode($LM_REQUEST["lastname"]) : "")."\" maxlength=\"32\" /></td>\n";
							$MESSAGE .= "	</tr>\n";
							...
							foreach($custom_fields as $custom_field) {
								if($custom_field["field_type"] == "linebreak") {
									...
									switch($custom_field["field_type"]) {
										case "textbox" :
											$MESSAGE .= "<input type=\"text\" id=\"".public_html_encode($custom_field["field_sname"])."\" name=\"".public_html_encode($custom_field["field_sname"])."\" value=\"".((isset($LM_REQUEST[$custom_field["field_sname"]])) ? public_html_encode($LM_REQUEST[$custom_field["field_sname"]]) : "")."\"".(((int) $custom_field["field_length"]) ? " maxlength=\"".(int) $custom_field["field_length"]."\"" : "")." />\n";
										break;
										case "textarea" :
											$MESSAGE .= "\t\t\t<textarea id=\"".public_html_encode($custom_field["field_sname"])."\" name=\"".public_html_encode($custom_field["field_sname"])."\" rows=\"4\" cols=\"30\">".((isset($LM_REQUEST[$custom_field["field_sname"]])) ? public_html_encode($LM_REQUEST[$custom_field["field_sname"]]) : "")."</textarea>\n";
										break;
										case "select" :
											if($custom_field["field_options"] != "") {
												...
												if((is_array($options)) && (count($options))) {
													...
													foreach($options as $option) {
														...
														$MESSAGE .= "<option value=\"".public_html_encode($pieces[0])."\"".((isset($LM_REQUEST[$custom_field["field_sname"]])) ? ((is_array($LM_REQUEST[$custom_field["field_sname"]])) ? ((in_array($pieces[0], $LM_REQUEST[$custom_field["field_sname"]])) ? " selected=\"selected\"" : "") : (($LM_REQUEST[$custom_field["field_sname"]] == $pieces[0]) ? " selected=\"selected\"" : "")) : "").">".public_html_encode($pieces[1])."</option>\n";
													}
											...
											if($custom_field["field_options"] != "") {
												...
												if((is_array($options)) && (count($options))) {
													foreach($options as $key => $option) {
														$pieces = explode("=", $option);
														$MESSAGE .= "<input type=\"checkbox\" id=\"".public_html_encode($custom_field["field_sname"])."_".$key."\" name=\"".public_html_encode($custom_field["field_sname"])."[]\" value=\"".public_html_encode($pieces[0])."\"".(((isset($LM_REQUEST[$custom_field["field_sname"]])) && (is_array($LM_REQUEST[$custom_field["field_sname"]])) && (in_array($pieces[0], $LM_REQUEST[$custom_field["field_sname"]]))) ? " checked=\"checked\"" : "")."> <label for=\"".public_html_encode($custom_field["field_sname"])."_".$key."\">".public_html_encode($pieces[1])."</label><br />\n";
													}
												}
											}
										break;
										case "radio" :
											if($custom_field["field_options"] != "") {
												...
												if((is_array($options)) && (count($options))) {
													foreach($options as $key => $option) {
														$pieces = explode("=", $option);
														$MESSAGE .= "<input type=\"radio\" id=\"".public_html_encode($custom_field["field_sname"])."_".$key."\" name=\"".public_html_encode($custom_field["field_sname"])."\" value=\"".public_html_encode($pieces[0])."\"".(((isset($LM_REQUEST[$custom_field["field_sname"]])) && ($LM_REQUEST[$custom_field["field_sname"]] == $pieces[0])) ? " checked=\"checked\"" : "")."> <label for=\"".public_html_encode($custom_field["field_sname"])."_".$key."\">".public_html_encode($pieces[1])."</label><br />\n";
													}
												}
											}
										break;
							 ...
							 */
							if(($config[ENDUSER_CAPTCHA] == "yes") && (!isset($_SESSION["php_captcha"]))) {
								$MESSAGE .= "	<tr>\n";
								$MESSAGE .= "		<td colspan=\"2\">\n";
							...
							if($config[ENDUSER_SUBCON] == "yes") {
								...
								if($result) {
									try {
										...
										$mail->Body		= str_replace(array("[name]", "[url]", "[abuse_address]", "[from]"), array(clean_input($LM_REQUEST["firstname"], "slashtestremove"), ($config[PREF_PUBLIC_URL].$config[ENDUSER_CONFIRM_FILENAME]."?id=".$result["confirm_id"]."&code=".$result["hash"]), $config[PREF_ABUEMAL_ID], $config[PREF_FRMNAME_ID]), $LANGUAGE_PACK["subscribe_confirmation_message"]);
								...
								if($result) {
									...
									if($result["success"] > 0) {
										if($config[ENDUSER_NEWSUBNOTICE] == "yes") {
			 								...
			 								$result	= $db->GetRow($query);
			 								if($result) {
				 								$notice_custom_data = get_custom_data($result["users_id"], array(), $config);
						...
						if(valid_address(trim($LM_REQUEST["email_address"]))) {
							...
							if(!$ERROR) {
								if($config[ENDUSER_UNSUBCON] == "yes") {
									$result = users_queue($LM_REQUEST["email_address"], "", "", $GROUP_IDS, array(), "usr-unsubscribe");
									if($result) {
										try {
											$mail			= new LM_Mailer($config);
									...
									if($config[ENDUSER_UNSUBNOTICE] == "yes") {
										$notice_custom_data = get_custom_data($SUBSCRIBER_IDS[0], array(), $config);
									}
