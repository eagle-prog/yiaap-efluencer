...
/*
	...
	$Id: profile.php 518 2011-03-10 21:58:15Z matt.simpson $
...
if(!@file_exists($LM_PATH."includes/config.inc.php")) {
	...
	if($config[ENDUSER_PROFILE] =="yes") {
		...
		if((!isset($LM_REQUEST["code"])) || (empty($LM_REQUEST["code"])) || (!isset($LM_REQUEST["id"])) || (empty($LM_REQUEST["id"]))) {
			/**
			 * Assume we are doing Step 1 if the code or ID field is not present or blank.
			 */ 
			if((($config[ENDUSER_CAPTCHA] == "no") || ((isset($LM_REQUEST["captcha_code"])) && (PhpCaptcha::Validate($LM_REQUEST["captcha_code"])))) && (isset($LM_REQUEST["email_address"])) && ($LM_REQUEST["email_address"] = clean_input($LM_REQUEST["email_address"], array("nows", "lowercase", "emailheaders"))) && (valid_address($LM_REQUEST["email_address"]))) {
				...
				if(($config[ENDUSER_CAPTCHA] == "yes") && (isset($LM_REQUEST["captcha_code"]))) {
					$MESSAGE .= "<thead>\n";
					$MESSAGE .= "	<tr>\n";
						...
						if(!$result["completed"]) {
							...
							if((is_array($USERS_IDS)) && (count($USERS_IDS))) {
								...
								if(($ERROR) || ($LM_REQUEST["action"] != "save")) {
									...
									$MESSAGE .= "	<tr>\n";
									$MESSAGE .= "		<td colspan=\"2\" style=\"text-align: right; padding-top: 5px\">\n";
									$MESSAGE .= "			<input type=\"button\" value=\"".(($SUCCESS) ? $LANGUAGE_PACK["page_profile_close_button"] : $LANGUAGE_PACK["page_profile_cancel_button"])."\" onclick=\"window.location='".$config[PREF_PUBLIC_URL].$config[ENDUSER_HELP_FILENAME]."'\" />\n";
									...
									foreach($custom_fields as $custom_field) {
										...
										if((!isset($LM_REQUEST[$custom_field["field_sname"]])) && (isset($custom_data["value"]))) {
