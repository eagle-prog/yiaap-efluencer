...
/*
	...
	$Id: unsubscribe.php 513 2011-03-09 05:38:08Z matt.simpson $
*/
...
if(!@file_exists($LM_PATH."includes/config.inc.php")) {
	...
	$LM_REQUEST["group_ids"]		= $LM_REQUEST["g"];
	...
	if((isset($LM_REQUEST["email_address"])) && ($LM_REQUEST["email_address"] = clean_input($LM_REQUEST["email_address"], array("nows", "lowercase", "emailheaders")))) {
		if(valid_address($LM_REQUEST["email_address"])) {
			...
			if((isset($LM_REQUEST["group_ids"])) && (!empty($LM_REQUEST["group_ids"]))) {
			...
			} elseif($LM_REQUEST["action"] != "confirm") {
				...
				switch($LM_REQUEST["action"]) {
					case "confirm" :
						if($config[ENDUSER_UNSUBCON] == "yes") {
							...
							if($result) {
								$query	= "INSERT INTO `".TABLES_PREFIX."confirmation` VALUES (NULL, '".time()."', 'usr-unsubscribe', ".$db->qstr($_SERVER["REMOTE_ADDR"], get_magic_quotes_gpc()).", ".$db->qstr($_SERVER["HTTP_REFERER"], get_magic_quotes_gpc()).", ".$db->qstr($_SERVER["HTTP_USER_AGENT"], get_magic_quotes_gpc()).", ".$db->qstr($LM_REQUEST["email_address"], get_magic_quotes_gpc()).", '', '', ".$db->qstr(serialize($group_ids)).", '', '', '0')";
								$db->Execute($query);
					...
					default:
						...
						foreach($groups_info as $group_id => $group_info) {
							...
							$MESSAGE .= "	<td style=\"padding-left: 5px\"><label for=\"lm_group_".$group_id."\">".str_replace(array("[email]", "[groupname]"), array(trim($LM_REQUEST["email_address"]), $group_info["name"]), $LANGUAGE_PACK["page_unsubscribe_list_groups"])."</label></td>";
							$MESSAGE .= "</tr>\n";
						}
