...
/*
	...
	$Id: archive.php 520 2011-03-11 20:58:47Z matt.simpson $
*/
	...
	if($config[ENDUSER_ARCHIVE] =="yes") {
		switch($LM_REQUEST["view"]) {
			case "html" :
				 ...
				 */
				if((isset($LM_REQUEST["id"])) && ($LM_REQUEST["id"] = clean_input($LM_REQUEST["id"], "nows"))) {
					if(is_array($pieces = explode(":", $LM_REQUEST["id"]))) {
						if((isset($pieces[0])) && ($tmp_input = clean_input($pieces[0], array("int")))) {
							$MESSAGE_ID	= $tmp_input;
						}
						if((isset($pieces[1])) && ($tmp_input = clean_input($pieces[1], array("int")))) {
							$QUEUE_ID	= $tmp_input;
						}
					}
				}
							...
							if(is_array($target)) {
								...
								if((is_array($target_groups)) && ($total_groups = count($target_groups))) {
									...
									foreach($target_groups as $group) {
										if (!isset($group["private"]) || $group["private"] == "true") {
											$private_groups++;
										}
									}
			...
			case "rss" :
				 ...
				 */
				if((isset($LM_REQUEST["f"])) && (array_key_exists($LM_REQUEST["f"] = clean_input($LM_REQUEST["f"], array("trim", "lower")), $VALID_FORMATS))) {
			...
			default :
				...
				if($MESSAGE_ID) {
					...
					if ($results) {
						foreach ($results as $result) {
							if (isset($result["target"]) && ($target = @unserialize($result["target"])) && is_array($target)) {
								$target_groups = groups_information($target);
