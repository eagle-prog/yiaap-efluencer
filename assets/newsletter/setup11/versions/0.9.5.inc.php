...
/*
	...
	$Id: 0.9.5.inc.php 481 2009-11-29 16:21:11Z matt.simpson $

	Requirements of 0.9.5 -> 2.2:
	=================================================
	New: cdata			Action: none (new table)
	New: cfields			Action: none (new table)
	New: confirmation		Action: upgrade "user_queue"
	New: groups			Action: upgrade "user_groups"
	New: messages			Action: upgrade "email_messages"
	New: preferences		Action: none (already complete)
	New: queue			Action: upgrade "email_queues"
	New: sending			Action: none (should be empty)
	New: templates			Action: upgrade "email_templates"
...
if($results) {
	foreach($results as $result) {
		...
		$record["target"]		= addslashes(serialize(array(trim($result["target"]))));
