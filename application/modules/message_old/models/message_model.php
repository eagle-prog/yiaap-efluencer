...
class message_model extends BaseModel {
    ...
    public function getMessage($user_id,$limit = '', $start = '' , $status=''){ 
        $this->db->select("*, IF(recipient_id = '$user_id', CONCAT(sender_id,'-',project_id), CONCAT(recipient_id,'-',project_id)) as msg_user", FALSE);
		$this->db->where("(recipient_id = $user_id OR sender_id = $user_id)");
		if(!empty($status)){
