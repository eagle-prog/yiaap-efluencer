...
<?php
		...
		echo '<p style="color:red;">'.__('message_no_favorite_message','No favorite message').' ..</p>';
	}
}
...
public function browse($project_id='', $other_user='') {
	if(!$this->session->userdata('user')){
		...
		$breadcrumb=array(
		array(
				'title'=>__('message','Message'),'path'=>''
			)
		);

		$data['breadcrumb']=$this->autoload_model->breadcrumb($breadcrumb,__('message','Message'));
	...
	if(count($message) > 0){
		foreach($message as $k => $v){
			...
			if(empty($project_name)){
				$project_name = __('message_project_deleted','Project Deleted');
			}
		...
		if($counter == 0){
			echo '<div class="info grey">'.__('message_no_message_found','No message found').'</div>';
		}
...
$breadcrumb=array(
array(
		'title'=>__('notification','Notification'),'path'=>''
)
);

$data['breadcrumb']=$this->autoload_model->breadcrumb($breadcrumb,__('notification','Notification'));
...
if($insert){
   ...
   redirect(base_url()."message/index/".$page.'/'.$sender_id."/".$project_id);
	...
	$this->session->set_flashdata('msg_failed',__('message_message_sending_failed',"Message sending failed"));
redirect(base_url()."message/index/".$page.'/'.$sender_id."/".$project_id);
//redirect(VPATH."message/");
}
}
...
{
echo __('message_no_conversation_yet',"No conversation yet..");
}
?>
</div>
</div>
