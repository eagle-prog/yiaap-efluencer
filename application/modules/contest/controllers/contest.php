...
class Contest extends MX_Controller {
	...
	public function post_contest(){
		...
		$apprivestatus=$this->auto_model->getFeild('verify','user','user_id',$usernew[0]->user_id);
		if($apprivestatus!='Y'){
			$this->session->set_flashdata('notApprove', __('you_cant_post_content_until_your_account_has_not_verified_by_admin','You can\'t post contest until your account has not verified by admin.'));
			 redirect(VPATH."dashboard/overview");
		}
		
		$data = array();
		$data['categories'] = $this->db->select('*')->where(array('parent_id' => 0 , 'status' => 'Y'))->get('skills')->result_array();
		//get_print($data['categories']); die();
		$breadcrumb=array(
			array(
				'title'=>__('contest','Contest'),'path'=>''
			)
		);
		$data['breadcrumb']=$this->autoload_model->breadcrumb($breadcrumb,__('contest','Contest'));
	...
	public function browse($c_id=''){
		...
		$breadcrumb=array(
            array(
                    'title'=>__('find_contest','FIND CONTEST'),'path'=>''
            )
        );
		$data['breadcrumb']=$this->autoload_model->breadcrumb($breadcrumb,__('contest','Contest'));
		...
		$data['categories'] = $this->db->select('*')->where(array('parent_id' => 0 , 'status' => 'Y'))->get('skills')->result_array();
		$skillid_new = $this->uri->segment(3);
		if(!empty($skillid_new))
		{
			$skill_id = $this->uri->segment(3);
			$data['skills'] = $this->db->select('id, skill_name, swedish_skill_name')->from('skills')->where('parent_id', $skill_id)->get()->result_array();
		} else {
			$data['skills'] = $this->db->select('id, skill_name, swedish_skill_name')->from('skills')->get()->result_array();
		}
		
		$q3 = array('select' => '*' , 'from' => 'city' , 'where' => '' , 'offset' => 1000);
		$data['cities'] =  get_results($q3);
			...
			if($this->form_validation->run()){
				...
				if($balance_check > 0){
					...
					if($wallet_balance < $balance_check){
						...
						$json['errors']['wallet'] = __('you_do_not_have_enough_balance_in_your_wallet','You do not have enough balance in your wallet '); 
						$json['status'] = 0;
				...
				if($error_count == 0){
					...
					if(!empty($contest_data['is_featured']) && ($contest_data['is_featured'] == 1)){
						...
						$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => $user_wallet_id, 'debit' => $feature_amount, 'ref' => $ref , 'info' => '{Featured_Contest}'));
						
						$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => PROFIT_WALLET, 'credit' => $feature_amount, 'ref' => $ref , 'info' => '{Featured_Contest}'));
					...
					if(!empty($contest_data['is_sealed']) && ($contest_data['is_sealed'] == 1)){
						...
						$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => $user_wallet_id, 'debit' => $sealed_amount, 'ref' => $ref , 'info' => '{Sealed_Contest}'));
						
						$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => PROFIT_WALLET, 'credit' => $sealed_amount, 'ref' => $ref , 'info' => '{Sealed_Contest}'));
			...
			if($this->form_validation->run()){
				...
				if($error_count == 0){
					...
					if(!empty($post['is_featured']) && ($post['is_featured'] == 1)){
						...
						$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => $user_wallet_id, 'debit' => $feature_amount, 'ref' => $ref , 'info' => '{Sealed_Contest}'));
						
						$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => PROFIT_WALLET, 'credit' => $feature_amount, 'ref' => $ref , 'info' => '{Sealed_Contest}'));
					...
					$notification = "$name {has_posted_an_entry_on_contest} <b>$contest_title </b>";
		...
		if(post() && $this->input->is_ajax_request() && $user_id){
			...
			if(($contest_owner == $user_id) && ($contest_row['status'] != 'C')){
				...
				if($wallet_balance < $budget){
					...
					$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => $user_wallet_id, 'debit' => $budget, 'ref' => $ref , 'info' => '{Contest_Awarded_To_Entry} #'.$entry_id));
					
					$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => $entry_user_wallet_id, 'credit' => $budget, 'ref' => $ref , 'info' => '{Contest_Awarded}'));
					...
					$notification = "{congratulation_you_have_been_awarded_for_the_contest} <b>$contest_title</b>";
	...
	public function notify_ajax(){
		...
		if(post() && $this->input->is_ajax_request() && $user_id){
			...
			$notification = "$name {will_work_on_an_entry_and_submit_before_end_for_the_contest} <b>$contest_title </b>";
			$link = 'contest/entries/'.$post['contest_id'];
			$this->notification_model->log($user_id, $to_user_id, $notification, $link);
			
			set_flash('succ_msg', __('contest_notification_send_to_contest_holder','Notification send to contest holder. Keep your entry ready to submit.'));
			$json['status'] = 1;
			echo json_encode($json);
		}
	...
	public function withdraw_contest(){
		...
		if(post() && $this->input->is_ajax_request() && $user_id){
			...
			set_flash('succ_msg', __('contest_entry_successfully_withdraw','Entry successfully withdraw'));
	...
	public function save_marker(){
		...
		if(post() && $this->input->is_ajax_request() && $user_id){
			...
			set_flash('succ_msg', __('contest_marker_successfully_added','Marker successfully added'));
	...
	public function save_marker_comment(){
		...
		if(post() && $this->input->is_ajax_request() && $user_id){
			...
			set_flash('succ_msg', __('contest_comment_successfully_added','comment successfully added'));
	...
	public function delete_marker(){
		...
		if(post() && $this->input->is_ajax_request() && $user_id){
			...
			set_flash('succ_msg', __('contest_marker_successfully_delete','Marker successfully delete'));
	...
	public function save_rating(){
		...
		if(post() && $this->input->is_ajax_request() && $user_id){
			...
			set_flash('succ_msg', __('contest_operation_successfully_complete','Operation successfully complete'));
