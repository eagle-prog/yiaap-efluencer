...
<section class="sec dashboard">
	...
	<div class="row">
		...
		<h4><?php echo $entries_count;?> <small><?php echo __('total_entry','Total Entries')?></small></h4>
		<?php }else{ ?>
		<h4 class="text-danger"><?php echo __('no_entry','No Entries found')?></h4>
		<?php } ?>
		</div>
		<div class="col-sm-3">
		<table style="width:100%;">
			<tr>
				<td><?php echo __('view','View')?>:</td>
				<td>
					<select class="form-control" onchange="performSearch()" name="view">
					<option value=""><?php echo __('all_entry','All Entries')?></option>
					<option value="ME" <?php echo (!empty($srch['view']) && $srch['view'] == 'ME') ? 'selected="selected"' : '';?>><?php echo __('my_entry','My Entries')?></option>
					<option value="A" <?php echo (!empty($srch['view']) && $srch['view'] == 'A') ? 'selected="selected"' : '';?>><?php echo __('active','Active')?></option>
					<option value="W" <?php echo (!empty($srch['view']) && $srch['view'] == 'W') ? 'selected="selected"' : '';?>><?php echo __('withdrawn','Withdrawn')?></option>
		...
		<table style="width:100%;">
			<tr>
				<td><?php echo __('sort_by','Sort by')?>:</td>
				<td>
					<select class="form-control" onchange="performSearch()" name="order_by">
					<option value=""><?php echo __('default','Default')?></option>
					<option value="recent" <?php echo (!empty($srch['order_by']) && $srch['order_by'] == 'recent') ? 'selected="selected"' : '';?>><?php echo __('recent','Recent')?></option>
					<option value="old" <?php echo (!empty($srch['order_by']) && $srch['order_by'] == 'old') ? 'selected="selected"' : '';?>><?php echo __('old','Old')?></option>
					</select>
				</td>
			</tr>
		</table>
	...
	<div class="row-10">
		...
		<?php if($is_withdraw){ ?>
		 ...
		 <div class="picture">
            ...
            <div class="hoverBlock">
          	<a href="javascript:void(0)" class="btn btn-site" onclick="confirm_repost('<?php echo $v['entry_id']; ?>')"><?php echo __('repost','Repost')?></a>
			</div>
			<?php } ?>
		...
		<?php if($is_sealed_entry && $is_auth_user){ ?>
		  <div class="picture">
			...
			<?php if($login_user_id == $v['user_id']){ ?>
            <div class="hoverBlock">
          	<a href="javascript:void(0)" class="btn btn-site" onclick="confirm_withdraw('<?php echo $v['entry_id']; ?>')"><?php echo __('withdrawn','Withdraw')?></a> <a href="javascript:void(0)" onclick="resale('<?php echo $v['entry_id']; ?>')" class="btn btn-grey"><?php echo __('sale_entry','Sale entry')?></a> </div>
			<?php } ?>
		  ...
		  <div class="card-body">
			...
			<?php if($login_user_id == $details['user_id']){ ?>
				...
				<a href="javascript:void(0)" onclick="awardContest('<?php echo $v['entry_id']; ?>', '<?php echo $v['contest_id']; ?>')" class="awardBtn btn btn-site"><?php echo __('award','Award')?></a>
				<?php } ?>
			
				<?php if($v['is_awarded'] == 1){ ?>
					<a href="javascript:void(0)" class="awardBtn btn btn-success"><?php echo __('awarded','Awarded')?></a>
				<?php }  ?>
		  ...
		  <?php if($login_user_id == $v['user_id']){ ?>
            <div class="hoverBlock">
          	<a href="javascript:void(0)" class="btn btn-site" onclick="confirm_withdraw('<?php echo $v['entry_id']; ?>')"><?php echo __('withdraw','Withdraw')?></a> <a href="javascript:void(0)" onclick="resale('<?php echo $v['entry_id']; ?>')" class="btn btn-grey"><?php echo __('sale_entry','Sale entry')?></a> </div>
			<?php } ?>
		  ...
		  <?php if($is_withdraw){ ?>
			<div><strong><?php echo __('withdraw','Withdraw')?></strong></div>
		  <?php }else{ ?>
		   ...
		   <a href="javascript:void(0)" class="<?php echo $is_user_liked > 0 ? 'active' : '';?>" onclick="toggleLike(this)" data-total-likes="<?php echo $total_likes; ?>" data-is-liked="<?php echo $is_user_liked; ?>" data-entry-id="<?php echo $v['entry_id']; ?>"><i class="zmdi zmdi-favorite zmdi-18x"></i> <span class="counter"><?php echo $total_likes; ?></span> <?php echo __('likes','Likes')?></a>
		   
		   <?php }else{  ?>
		   
		   <div><strong><?php echo __('sealed','Sealed')?></strong></div>
	...
	<div class="row">
		<div class="col-sm-12 col-md-8">
			...
			<div class="white shadow_1">
				<div class="panel-heading"><h4><?php echo __('contest_public_clarification_board','Public Clarification Board')?></h4></div>
				<div class="media comment-box mt-0">
					...
					<div class="media-body">
						<div id="comment-form-wrapper">
							...
							<textarea class="form-control" name="comment" id="comment-input" rows="3" placeholder="<?php echo __('contest_write_a_comment','Write a comment')?>" onkeypress="check_key(event)"></textarea>
							<input type="hidden" name="contest_id" value="<?php echo $contest_id; ?>"/>
							<p class="text-muted f-12"><?php echo __('contest_request_contest_holder_to_increase_prize','Request contest holder to increase prize.')?></p>
							<button class="hide btn btn-primary" id="commentSubmitBtn"><?php echo __('post','Post')?></button>
							</form>
						</div>
					</div>
					<div class="clearfix"></div>
							...
							<div class="media comment-box mt-0 reply-box" id="reply_comment_wrapper_<?php echo $v['comment_id'];?>" style="display:none;">
								...
								<div class="media-body">
									<div id="reply_comment_form_<?php echo $v['comment_id'];?>">
										...
										<button class="hide btn btn-primary" id="submit-cmnt-btn-child"><?php echo __('post','Post')?></button>
										</form>
									</div>
								</div>
								<div class="clearfix"></div>
        ...
        <div class="panel panel-default">
			<div class="panel-heading"><h4><?php echo __('contest_similar_contests','Similar Contests')?></h4></div>
...
bootbox.confirm({
		title:"<?php echo __('contest_Confirm','Confirm')?> ",
		message: "<?php echo __('contest_are_you_sure','Are you sure to withdraw this entry')?> ? <br/></br/>",
	
		buttons: {
			confirm: {
				label: "<?php echo __('contest_Yes','Yes')?>",
				className: 'btn-success'
			},
			cancel: {
				label: "<?php echo __('contest_No','No')?>",
...
function confirm_repost(entry_id){
	bootbox.confirm({
		title:"<?php echo __('contest_Confirm','Confirm')?> ",
		message: "<?php echo __('contest_are_you_sure_repost','Are you sure to repost this entry')?> ? <br/></br/>",
...
function resale(entry_id){
	bootbox.prompt({
		title: "<?php echo __('contest_Enter_amt','Enter your sale amount for this entry')?>",
		inputType: 'text',
		callback: function (result) {
