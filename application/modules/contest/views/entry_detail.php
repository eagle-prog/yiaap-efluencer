...
<div class="row">
            ...
            <aside class="col-sm-4">
				...
				<div>
					<a href="javascript:void(0)" onclick="toggleMarker()"><?php echo __('contest_add_marker','Add marker')?></a>
				</div>
				<hr/>
				<?php } ?>
                ...
                <div class="choose_favourite">
                  ...
                  <div class="pull-right"><i class="zmdi zmdi-share zmdi-18x"></i> <?php echo __('share','Share')?>:
...
<script id="entry-comment-form" type="text/template">
	<form onsubmit="postCommentAjax(this, event)">
		<textarea class="form-control" name="comment" rows="3" placeholder="<?php echo __('contest_write_a_comment','Write a comment')?>" onkeypress="check_key_entry(event)"></textarea>
		<input type="hidden" name="entry_id" value="<?php echo $entry_detail['entry_id'];?>"/>
		<button id="entryCommentSubmitBtn" class="hidden"><?php echo __('send','Send')?></button>
	</form>
</script>
...
(function(){
	...
	var Marker = function(ele){
		...
		this.add = function(p){
			...
			var inp_form = $('<input type="text" class="form-control input-sm" placeholder="<?php echo __('contest_write_your_comment_here','Write your comment here')?>"/>');

			var content_tmpl = $('<div class="comment_wrapper"></div>');
			var pop_header = $('<div class="pop_header"><span><?php echo __('comments','Comments')?></span></div>');
			...
			pop_delete.on('click', function(e){
				e.preventDefault();
				var c = confirm('<?php echo __('are_you_sure_to_delete_this_marker','Are you sure to delete this marker')?> ? ');
